...
void CCat::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects) {
	...
	if (state == CAT_STATE_IDLE && run_start == 0 && ((gimmick->x - x <= CAT_LIMIT_X + CAT_BBOX_WIDTH && gimmick->x - x >= 0) || (x - gimmick->x <= CAT_LIMIT_X && x - gimmick->x >= 0)) && ((gimmick->y - y <= CAT_LIMIT_Y && gimmick->y - y >= 0) || (y - gimmick->y <= CAT_LIMIT_Y + CAT_BBOX_HEIGHT && y - gimmick->y >= 0)))
	{
		sleep_start = 0;
		StartRun();
	}
	...
	if (state == CAT_STATE_CHASE)
	{
		...
		else if (x >= gimmick->x + CAT_LIMIT_RUN_X + CAT_BBOX_WIDTH)
		{
			...
			SetState(CAT_STATE_CHASE);
		if (stop_star == 1 && star->state == STAR_STATE_READY)
		{
			stop_star = 0;
